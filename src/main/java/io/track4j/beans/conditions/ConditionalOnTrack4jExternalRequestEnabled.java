...
public class ConditionalOnTrack4jExternalRequestEnabled implements Condition {
    private final Track4jProperties track4jProperties;

    public ConditionalOnTrack4jExternalRequestEnabled() {
    ...
    public boolean matches(ConditionContext ctx, AnnotatedTypeMetadata md) {
        if (md.isAnnotated(ConditionalOnTrack4jExternalEnabled.class.getName())) {
            return track4jProperties.isEnabled() && track4jProperties.isExternalRequestTrackingEnabled();
        }
        return false;
    }
}